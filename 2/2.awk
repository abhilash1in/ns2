...

{
	...
	if(event == "d" && pkttype == "tcp" && fromnode == "0.0" && tonode == "3.0")
	{
		droppedtcp1++;
	}
	if(event == "d" && pkttype == "tcp" && fromnode == "1.0" && tonode == "3.1")
	{
		droppedtcp2++;
	}
}
...
END {
	...
	printf("The number of FTP packets dropped is %d \n", droppedtcp1);
	printf("The number of TELNET packets dropped is %d \n", droppedtcp2);
}
